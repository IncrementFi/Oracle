...
const binanceURL_one = "https://api.binance.us/api/v3/ticker/price?symbol="
const binanceURL_all = "https://api.binance.us/api/v3/ticker/price"
...
async function requestOne(pair) {
	...
	try {
		...
		throw( Error("binanceUS parse request error["+url+"]") )
	}
...
async function requestAll(pairs) {
	...
	if (Object.keys(pairs).length > 3 ) {
		...
		try {
			...
			throw( Error("binanceUS parse request result json error.") )
		}
	} else {
	...
	if (respJson.length > 0) {
		...
		throw(Error("binanceUS respJson error."));
	}
	return res
}
...
async function parseResponse(respJson, pairs) {
	...
	for (let pair in pairs) {
		...
		if (!(pairWithoutSlash in respMap)) {
			throw(Error("binanceUS pair resp mismatch error: "+pair));
		}
...
//(async () => { await requestAll({ "USDT/USD":0.0, "BTC/USDT": 0.0 })	})()
